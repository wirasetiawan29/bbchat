...
class CallViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.isAudioOnlyCall {
            ...
            self.videoToggleButton.setImage(UIImage(named: "vc.slash.fill", in: .bookingUI, with: UIImage.SymbolConfiguration(pointSize: 16, weight: .regular)), for: .normal)
    ...
    @IBAction func didToggleCamera(_ sender: Any) {
        let newimg = UIImage(named: cameraManager.isCapturing ? "vc.slash.fill" : "vc.fill", in: .bookingUI, with: UIImage.SymbolConfiguration(pointSize: 16, weight: .regular))
...
//        #endif
    ...
    private func playSoundEffect(_ effect: String?, loop: Bool = false) {
        ...
        let path = Bundle.bookingUI.path(forResource: "\(effect).m4a", ofType: nil)!
