...
public class SharedUtils {
    ...
    public static func saveImageName(name: String) {
        SharedUtils.kAppDefaults.set(name, forKey: "imageFileName")
    }
    
    public static func getImageName() -> String {
        return SharedUtils.kAppDefaults.string(forKey: "imageFileName") ?? ""
    }
    
    public static func removeImageName() {
        SharedUtils.kAppDefaults.removeObject(forKey: "imageFileName")
    }
...
extension Tinode {
    public static func getConnectionParams() -> (String, Bool) {
        ...
        switch environment {
        case .production:
            return ("oapi-tinode.bluebird.id:443", true)
        case .regress, .dev, .staging:
