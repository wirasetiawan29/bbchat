...
public class MessageViewController: UIViewController {
    ...
    private enum Constants {
        ...
        static let kAdditionalP2PVerticalCellSpacing: CGFloat = 10
    ...
    /// The `predefinedChat` is used as the `inputAccessoryView` in the view controller.
    lazy var predefinedChat: PredefinedChatView = {
        let view = PredefinedChatView()
        view.autoresizingMask = .flexibleHeight
        return view
    }()
    ...
    private lazy var navBarBackBtn: UIBarButtonItem = {
        
        let closeIcon = UIImage(named: "ic-arrow-left-black", in: .bookingUI, compatibleWith: nil)
        
        return UIBarButtonItem(
            image: closeIcon,
            style: .plain, target: self, action: #selector(backButton(sender:)))
    }()
    ...
    let backgroundImageView: UIImageView = {
       let theImageView = UIImageView()
        theImageView.image = UIImage(named: "chat_background", in: .bookingUI, with: nil)
       theImageView.translatesAutoresizingMaskIntoConstraints = false //You need to call this property so the image is added to your view
       return theImageView
    }()
    
    @objc func backButton(sender: UIMenuController) {
    ...
    var prefedinedBottomAnchor: NSLayoutConstraint!

    var interactor: (MessageBusinessLogic & MessageDataStore)?
    private let refreshControl = UIRefreshControl()
    
    var heightPredefinedConstraint:NSLayoutConstraint? = nil
    var isHidePredefinedChat: Bool = false
    ...
    private func setup() {
        ...
        self.imagePicker = ImagePicker(presentationController: self, delegate: self, editable: false, allowVideo: false)
    ...
    public override func loadView() {
        ...
        let predefined = PredefinedChatView()
        predefined.sizeToFit()
        predefined.layer.cornerRadius = 10
        predefined.layer.shadowOpacity = 0.10
        buttonGoToLatest.layer.shadowOffset = CGSize()
        
        predefined.onDidFirstTextSelection = { [weak self] string in
            self?.sendMessageBar.delegate?.sendMessageBar(sendText: string)
        }
        
        predefined.onDidSecondTextSelection = { [weak self] string in
            self?.sendMessageBar.delegate?.sendMessageBar(sendText: string)
        }
        
        predefined.onDidThirdTextSelection = { [weak self] string in
            self?.sendMessageBar.delegate?.sendMessageBar(sendText: string)
        }
        
        predefined.onDidHideContent = { [weak self] hide in
            DispatchQueue.main.async {
                self?.calculatePredefinedHeight(hide: hide)
            }
        }
        
        view.addSubview(predefined)
        self.predefinedChat = predefined
        self.predefinedChat.translatesAutoresizingMaskIntoConstraints = false
        
        self.prefedinedBottomAnchor = self.predefinedChat
            .bottomAnchor
            .constraint(equalTo: view.bottomAnchor,
                        constant: -(inputAccessoryView?.frame.height ?? 0) - 100)
        
        heightPredefinedConstraint = self.predefinedChat.heightAnchor.constraint(equalToConstant: 180)
        heightPredefinedConstraint?.isActive = true
        NSLayoutConstraint.activate([
            self.predefinedChat.widthAnchor.constraint(equalToConstant: 280.0),
            self.prefedinedBottomAnchor,
            self.predefinedChat.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -8)
        ])
    }
    
    private func calculatePredefinedHeight(hide: Bool) {
        self.predefinedChat.translatesAutoresizingMaskIntoConstraints = false
        isHidePredefinedChat = hide
        if hide {
            UIView.animate(withDuration: 1.0, animations: {
                self.heightPredefinedConstraint?.constant = 45
                self.heightPredefinedConstraint?.isActive = true
                self.view.layoutIfNeeded()
            })
        } else {
            UIView.animate(withDuration: 1.0, animations: {
                self.heightPredefinedConstraint?.constant = 180
                self.heightPredefinedConstraint?.isActive = true
                self.view.layoutIfNeeded()
            })
        ...
        minimizePredefinedChat(hide: hide)
    }
    
    func configPredefinedKeyboard(hide: Bool) {
        self.predefinedChat.translatesAutoresizingMaskIntoConstraints = false
        self.prefedinedBottomAnchor.isActive = false
        if hide {
            self.prefedinedBottomAnchor = self.predefinedChat
                .bottomAnchor
                .constraint(equalTo: view.bottomAnchor,
                            constant: -(inputAccessoryView?.frame.height ?? 0) - 10)
            self.prefedinedBottomAnchor.isActive = true
//            heightPredefinedConstraint = self.predefinedChat.heightAnchor.constraint(equalToConstant: 180)
//            heightPredefinedConstraint?.isActive = true
            NSLayoutConstraint.activate([
                self.predefinedChat.widthAnchor.constraint(equalToConstant: 280.0),
            ...
            self.prefedinedBottomAnchor = self.predefinedChat
                .bottomAnchor
                .constraint(equalTo: view.bottomAnchor,
                            constant: -(inputAccessoryView?.frame.height ?? 0) - 350)
            self.prefedinedBottomAnchor.isActive = true
//            heightPredefinedConstraint = self.predefinedChat.heightAnchor.constraint(equalToConstant: 180)
//            heightPredefinedConstraint?.isActive = true
            NSLayoutConstraint.activate([
    ...
    public override func viewDidLayoutSubviews() {
        ...
        minimizePredefinedChat(hide: isHidePredefinedChat)
    ...
    func minimizePredefinedChat(hide: Bool) {
        
        if hide {
            let height = inputAccessoryView?.frame.height ?? 0
            self.collectionViewBottomAnchor.constant = -(height+50.0)
            self.goToLatestButtonBottomAnchor.isActive = true
    ...
    @objc func sendAttachment(notification: NSNotification) {
        ...
        case let content as ImagePreviewContent:
            ...
            let imageFileName = SharedUtils.getImageName()
            
            if imageFileName != content.fileName && data.count > 0 {
                if data.count > maxInbandSize {
                    ...
                    SharedUtils.saveImageName(name: content.fileName ?? "")
                    break
                } else {
    ...
    @objc func navBarCallTapped(sender: UIMenuController) {
        ...
        let tinode = Cache.tinode
        var topic = tinode.getTopic(topicName: callRoomId!) as? DefaultComTopic
        if topic == nil {
            topic = tinode.newTopic(for: callRoomId!) as? DefaultComTopic
        }
        
        callViewController.topic = topic
...
extension MessageViewController: MessageDisplayLogic {
    ...
    func updateTitleBar(pub: TheCard?, online: Bool?, deleted: Bool) {
        ...
        navigationItem.titleView = leftTitle(fullName ?? "Undefined")
        ...
        // commented until voip feature ready
//        self.navigationItem.setRightBarButtonItems(items, animated: false)
        
        self.navigationItem.setLeftBarButtonItems(backItems, animated: true)
    }
...
extension MessageViewController: MessageViewLayoutDelegate {
    ...
    func calcCellSize(forItemAt indexPath: IndexPath) -> CGSize {
        ...
        var height = calcCellHeightFromContent(for: message, at: indexPath, containerHeight: containerHeight, avatarsVisible: hasAvatars, progressVisible: showProgress)
        ...
        let size = CGSize(width: calcCellWidth(), height: height)
