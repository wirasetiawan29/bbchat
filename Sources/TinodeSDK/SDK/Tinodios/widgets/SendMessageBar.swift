...
class SendMessageBar: UIView {
    ...
    private enum Constants {
        ...
        static let kSendButtonImageWave = UIImage(named: "img_microphone", in: .bookingUI, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsNormal))!
        static let kSendButtonImageWavePressed = UIImage(named: "img_microphone", in: .bookingUI, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsPressed))!
        static let kSendButtonImageArrow = UIImage(named: "img_send", in: .bookingUI, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsNormal))!
//        static let kSendButtonImageEditCheck = UIImage(systemName: "checkmark.circle.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsNormal))
        ...
        static let kSendButtonImageEditCheck = UIImage(named: "img_send", in: .bookingUI, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsNormal))!
...
extension SendMessageBar: UITextViewDelegate {
    ...
    func showMessageButton() {
        inputField.isShowingPlaceholder = true
        let textdata = self.inputField.placeholderText
        delegate?.sendMessageBar(textChangedTo: textdata)
        inputField.text = textdata
        inputField.textColor = inputField.mainTextColor
        let size = CGSize(width: frame.width - Constants.inputFieldInsetLeading - Constants.inputFieldInsetTrailing, height: .greatestFiniteMagnitude)
        let fittingSize = inputField.sizeThatFits(size)
        if fittingSize.height <= inputFieldMaxHeight {
            inputFieldHeight.constant = fittingSize.height + 2 // Not sure why but it seems to be off by 2
