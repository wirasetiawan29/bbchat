...
//        Cache.log.info("Starting outgoing call (uuid: %@) on topic: %@", self.callInProgress!.uuid.uuidString, topicName)
    ...
    public func displayIncomingCall(uuid: UUID, onTopic topicName: String, originatingFrom fromUid: String, withSeqId seq: Int, audioOnly: Bool, completion: ((Error?) -> Void)?) {
        guard self.callInProgress == nil else {
    ...
    public func dismissIncomingCall(onTopic topic: String, withSeqId seq: Int) {
